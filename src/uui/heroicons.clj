...
(comment
  ...
  (defn to-hiccup [f]
    (-> (slurp f)
        ...
        (nth 3)
        (nth 2)
        (update 1 assoc :class ['(or size "size-4")])
        (pr-str)))

  (def nms-1  (into #{} (for [f (->> (file-seq (io/file "tmp/heroicons/src/24/solid/"))
                                     (sort-by #(.getName %)))]
                          (when (str/ends-with? (.getName f) ".svg")
                            (str (str/replace (.getName f) #".svg$" ""))))))

  (spit "/tmp/icons.clj"
         (with-out-str 
           (doseq [n (sort nms-1)]
             (println "(defn" n "[& args] (apply icon " (pr-str n) "args))"))))

  (clojure.set/difference nms-1 nms-2)
  (clojure.set/difference nms-1 nms-3)

  (def s1
    (with-out-str
      (doseq [f (->> (file-seq (io/file "tmp/heroicons/src/16/solid/"))
                     (sort-by #(.getName %)))]
        (when (str/ends-with? (.getName f) ".svg")
          (println "(defn " (str (str/replace (.getName f) #".svg$" "") "-xs") "[& [size]]" (to-hiccup f) ")")))))

  (def s1
    (with-out-str
      (doseq [f (->> (file-seq (io/file "tmp/heroicons/src/20/solid/"))
                     (sort-by #(.getName %)))]
        (when (str/ends-with? (.getName f) ".svg")
          (println "(defn " (str (str/replace (.getName f) #".svg$" "") "-sm") "[& [size]]" (to-hiccup f) ")")))))

  (def s1
    (with-out-str
      (doseq [f (->> (file-seq (io/file "tmp/heroicons/src/24/solid/"))
                     (sort-by #(.getName %)))]
        (when (str/ends-with? (.getName f) ".svg")
          (println "(defn " (str (str/replace (.getName f) #".svg$" "")) "[& [size]]" (to-hiccup f) ")")))
      ))

  (def s1
    (with-out-str
      (doseq [f (->> (file-seq (io/file "tmp/heroicons/src/24/outline/"))
                     (sort-by #(.getName %)))]
        (when (str/ends-with? (.getName f) ".svg")
          (println "(defn " (str (str/replace (.getName f) #".svg$" "-o")) "[& [size]]" (to-hiccup f) ")")))
      ))

  (into [] (for [f (->> (file-seq (io/file "tmp/heroicons/src/24/solid/"))
                        (sort-by #(.getName %)))]
             (when (str/ends-with? (.getName f) ".svg")
               (str (str/replace (.getName f) #".svg$" "")))))

  (spit "src/uui/heroicon.edn" s1)

  (file-seq (io/file "tmp/heroicons/src/20/solid/"))

  (file-seq (io/file "tmp/heroicons/src/24/outline"))
